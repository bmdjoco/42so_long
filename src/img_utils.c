...
/*   Updated: 2025/07/13 12:04:01 by bdjoco           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_img_win(t_game *game, int x, int y, char c)
{
	...
	mlx_put_image_to_window(game->mlx, game->win, img, x * TYLE_SIZE, y *TYLE_SIZE);
}
...
void	is_drop_case(t_game *game, int x, int y)
{
	...
	game->score++;
	game->carte->map[y_pos][x_pos].img = get_image(game, game->carte->map[y_pos][x_pos].s);
